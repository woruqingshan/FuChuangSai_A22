...
export function createAvatarPanel({ onProfileChange } = {}) {
  ...
  element.innerHTML = `
    ...
    <div class="avatar-source-controls">
      ...
      <p class="avatar-source-meta" data-role="avatar-source-meta">
        The default avatar is active. Connect an external avatar page or video URL here.
      </p>
    </div>
  ...
  function updateProfileUi(profile) {
    ...
    profileChip.textContent = profile.name.includes("B") ? "Casual Companion" : "Supportive Companion";
  ...
  function disconnectExternalStream(message = "External source disconnected. The default avatar is active.") {
    renderer.clearPinnedVideoSource();
    face.dataset.externalPage = "off";
    setEmbedFrame("");
    persistStreamUrl("");
  ...
  avatarVideo?.addEventListener("loadeddata", () => {
    ...
    setSourceUi({
      chipText: "External Video",
      metaText: `Media stream connected: ${activeUrl}`,
      connected: true,
    });
  });
  ...
  setSourceUi({
    chipText: "Default Avatar",
    metaText: "The default avatar is active. Connect an external avatar page or video URL here.",
    connected: false,
  });
  ...
  if (!sourceControlsEnabled) {
    disconnectExternalStream("The default avatar is active.");
  }
